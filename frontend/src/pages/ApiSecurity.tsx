...
export const ApiSecurity: React.FC<Props> = ({ onNavigate }) => {
  ...
  const handlePasteSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to parse raw specification.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleScanApi = () => {
    if (!targetBaseUrl.trim()) {
      alert('Please specify the API base URL to test against.');
      return;
    }
    // Navigate to new-scan pre-filled with api_spec
    onNavigate('new-scan');
  };
        ...
        <div className="bg-[#111726] border border-[#1d273a] p-6 rounded-xl space-y-4">
          ...
          {inputMode === 'upload' ? (
            <div className="border-2 border-dashed border-[#232f44] hover:border-blue-500/50 rounded-xl p-12 text-center transition-colors bg-[#0d121c]">
              ...
              <label className="mt-4 inline-flex items-center px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium cursor-pointer transition-colors">
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-[#1d273a] text-gray-300">
                  {analysis.endpoints.map((ep, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-[#151c2d] transition-colors">
