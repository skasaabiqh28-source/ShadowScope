...
export const CompareScans: React.FC = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#111726] border border-[#1d273a] p-5 rounded-xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-400 block mb-1">Baseline Scan (Scan 1)</label>
            <select
              value={scan1Id}
          ...
          <div>
            <label className="text-xs text-gray-400 block mb-1">Comparison Target (Scan 2)</label>
            <select
              value={scan2Id}
        ...
        <div className="flex justify-end">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-800 text-white text-xs font-medium rounded-lg transition-colors"
          >
          ...
          {diffResult.resolved.length > 0 && (
            <div className="bg-[#111726] border border-[#1d273a] p-5 rounded-xl space-y-3">
              <h3 className="text-sm font-semibold text-emerald-400 flex items-center gap-2">
                ...
                Resolved Vulnerabilities ({diffResult.resolved.length})
              </h3>
              <div className="space-y-2">
                {diffResult.resolved.map((f: Finding) => (
                  <div key={f.id} className="p-3 bg-[#0e1320] border border-[#1d273a] rounded-lg text-xs flex justify-between items-center">
                    <div className="space-y-0.5">
                      <div className="font-semibold text-gray-200">{f.title}</div>
                      <div className="text-gray-400 font-mono text-[11px]">{f.location || 'N/A'}</div>
